...
public class RegistrySetBuilder {
    ...
    static <T> HolderLookup.RegistryLookup<T> lookupFromMap(
        final ResourceKey<? extends Registry<? extends T>> p_312781_, final Lifecycle p_312783_, final Map<ResourceKey<T>, Holder.Reference<T>> p_312180_
    ) {
    ...
    private static HolderLookup.Provider buildProviderWithContext(RegistryAccess p_312694_, Stream<HolderLookup.RegistryLookup<?>> p_312024_) {
        Stream<HolderLookup.RegistryLookup<?>> stream = p_312694_.registries().map(p_258195_ -> p_258195_.value().asLookup());
        return HolderLookup.Provider.create(Stream.concat(stream, p_312024_));
    }
    ...
    private HolderLookup.Provider createLazyFullPatchedRegistries(
        ...
        List<HolderLookup.RegistryLookup<?>> list = p_312890_.keySet()
            .stream()
            .map(
                p_311471_ -> this.createLazyFullPatchedRegistries(registrysetbuilder$compositeowner, p_312464_, p_311471_, p_311825_, p_311942_, mutableobject)
            )
            .peek(registrysetbuilder$compositeowner::add)
            .collect(Collectors.toUnmodifiableList());
        HolderLookup.Provider holderlookup$provider = buildProviderWithContext(p_312301_, list.stream());
    ...
    static class LazyHolder<T> extends Holder.Reference<T> {
        ...
        protected LazyHolder(HolderOwner<T> p_312861_, @Nullable ResourceKey<T> p_312263_) {
            super(Holder.Reference.Type.STAND_ALONE, p_312861_, p_312263_, (T)null);
        }
    ...
    static record RegistryContents<T>(
        ...
        public HolderLookup.RegistryLookup<T> buildAsLookup(RegistrySetBuilder.CompositeOwner p_312295_) {
                    ...
                    Collectors.toUnmodifiableMap(
                        ...
                        p_311927_ -> {
                            ...
                            Holder.Reference<T> reference = valueandholder.holder()
                                .orElseGet(() -> Holder.Reference.createStandAlone(p_312295_.cast(), (ResourceKey<T>)p_311927_.getKey()));
