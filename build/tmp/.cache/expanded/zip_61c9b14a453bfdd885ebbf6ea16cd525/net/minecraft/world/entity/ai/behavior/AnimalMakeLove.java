...
public class AnimalMakeLove extends Behavior<Animal> {
    ...
    private Optional<? extends Animal> findValidBreedPartner(Animal p_22432_) {
        return p_22432_.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().findClosest(p_311583_ -> {
            if (p_311583_.getType() == this.partnerType && p_311583_ instanceof Animal animal && p_22432_.canMate(animal) && !animal.isPanicking()) {
                return true;
            }
