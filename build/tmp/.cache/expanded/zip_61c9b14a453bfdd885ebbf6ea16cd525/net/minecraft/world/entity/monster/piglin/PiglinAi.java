...
public class PiglinAi {
    ...
    public static void angerNearbyPiglins(Player p_34874_, boolean p_34875_) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !p_34875_ || BehaviorUtils.canSee(p_34881_, p_34874_)).forEach(p_311688_ -> {
            if (p_311688_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(p_311688_, p_34874_);
            } else {
                setAngerTarget(p_311688_, p_34874_);
            }
        });
    }
    ...
    protected static void wasHurtBy(Piglin p_34838_, LivingEntity p_34839_) {
        if (!(p_34839_ instanceof Piglin)) {
            ...
            getAvoidTarget(p_34838_).ifPresent(p_308764_ -> {
                if (p_308764_.getType() != p_34839_.getType()) {
    ...
    protected static void broadcastAngerTarget(AbstractPiglin p_34896_, LivingEntity p_34897_) {
        getAdultPiglins(p_34896_).forEach(p_308761_ -> {
            if (p_34897_.getType() != EntityType.HOGLIN || p_308761_.canHunt() && ((Hoglin)p_34897_).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(p_308761_, p_34897_);
            }
        });
    }
