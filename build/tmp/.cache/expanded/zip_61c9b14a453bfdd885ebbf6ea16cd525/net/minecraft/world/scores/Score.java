...
public class Score implements ReadOnlyScoreInfo {
    ...
    public static Score read(CompoundTag p_313855_) {
        Score score = new Score();
        score.value = p_313855_.getInt("Score");
        score.locked = p_313855_.getBoolean("Locked");
        if (p_313855_.contains("display", 8)) {
            score.display = Component.Serializer.fromJson(p_313855_.getString("display"));
        }

        if (p_313855_.contains("format", 10)) {
            NumberFormatTypes.CODEC.parse(NbtOps.INSTANCE, p_313855_.get("format")).result().ifPresent(p_313664_ -> score.numberFormat = p_313664_);
        }
